...
class DDIMSampler(object):
    ...
    def add_noise(self, x, noise, alpha_index):
        alpha = self.ddim_alphas[alpha_index]      # 0.999--> 0.005, 表示的是 信号的强度 ;([64])，对应去噪 64步; 相反 sampler.ddim_timesteps： 0——> 999； 表示还需要进行的 去噪步数
        beta = 1 - alpha
        latents = alpha**(0.5) * x + beta**(0.5)*noise
        return latents

    def continue_add_noise(self, latents, current_alpha_index, num_steps):
        for _ in range(num_steps):
            next_alpha_index = current_alpha_index + 1
            if next_alpha_index >= len(self.ddim_alphas):
                raise ValueError("Exceeded maximum alpha index")
            
            # 获取当前和下一步的alpha值
            current_alpha = self.ddim_alphas[current_alpha_index]
            next_alpha = self.ddim_alphas[next_alpha_index]
            
            # 计算噪声混合参数
            sqrt_ratio = torch.sqrt(next_alpha / current_alpha)
            sqrt_one_minus_ratio = torch.sqrt(1 - (next_alpha / current_alpha))
            
            # 生成新的噪声并更新隐变量
            new_noise = torch.randn_like(latents)
            latents = sqrt_ratio * latents + sqrt_one_minus_ratio * new_noise
            
            current_alpha_index = next_alpha_index
